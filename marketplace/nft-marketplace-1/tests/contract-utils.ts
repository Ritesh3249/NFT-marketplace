...
export function createTokenEventsEvent(uid: BigInt): TokenEvents {
  let tokenEventsEvent = changetype<TokenEvents>(newMockEvent())

  tokenEventsEvent.parameters = new Array()

  tokenEventsEvent.parameters.push(
    new ethereum.EventParam("uid", ethereum.Value.fromUnsignedBigInt(uid))
  )

  return tokenEventsEvent
}
